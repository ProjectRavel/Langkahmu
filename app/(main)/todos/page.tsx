...
import { Calendar, Clock, Tag, StickyNote, Pencil, Trash2 } from "lucide-react";
...
export default function MyDay() {
  ...
  return (
    <section className="w-full min-h-screen bg-black text-white px-4 py-4">
      {/* Header */}
      <div className="mb-4">
        <h1 className="text-xl font-bold flex items-center gap-2">📝 To Do</h1>
      ...
      <div className="mb-4">
        ...
        <Collapsible open={open}>
          <CollapsibleContent className="animate-slide-down mt-3 flex gap-2 flex-wrap">
            <Button
              variant="outline"
              className="bg-[#2d2d2d] text-white border-[#444] h-8 text-xs px-3"
            >
                  ...
                  >
                    ...
                    <div className="flex gap-2 ml-2 items-center">
                      <Select
                        value={task.category}
                        onValueChange={(val) =>
                          changeTaskCategory(task.id, val as Category)
                        }
                      >
